CREATE TABLE `task_online` (
  ...
  `handlers` text CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL COMMENT 'handler信息',
  `reader_id` bigint(20) NOT NULL COMMENT '数据源信息表id',
  `reader_type` tinyint(4) NOT NULL COMMENT '来源表的类型',
  `reader_table_name` text NOT NULL COMMENT '来源表表名',
...
-- ALTER TABLE `task_online` MODIFY COLUMN handlers text NOT NULL COMMENT 'handler信息';
-- ALTER TABLE `task_online` MODIFY COLUMN `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP  COMMENT '创建时间', MODIFY COLUMN `modify_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间'

ALTER TABLE `task_online` ADD COLUMN `reader_url` text NOT NULL DEFAULT "" COMMENT '来源数据源的url，多数据源通过；分隔';
ALTER TABLE `task_online` ADD COLUMN `writer_url` VARCHAR(255) NOT NULL DEFAULT "" COMMENT '去向数据源的url';