...
object DDBDBIParser extends Parser {
  override def parseReader(readerData: JValue): Unit = {
    val prefix = readerPrefix("ddb.dbi")
    implicit val formats: DefaultFormats.type = DefaultFormats
    val properties = Properties.properties
    addSparkConf(properties, "spark.transmit.reader", "com.netease.music.da.transfer.ddb.dbi.reader.DDBDBIReader")
    ...
    val version = (datasource \ "connectionInformation" \ "version").extract[String]
    ...
    val ddbCommonJars =
      FileUtils.listFiles(FileUtils.getFile(workerHome, "ddb-dbi"), null, false)
        .asScala.filter(!_.isDirectory).map(_.getAbsolutePath).filter(_.endsWith("jar"))
    val ddbVersionJars =
      FileUtils.listFiles(FileUtils.getFile(workerHome, "ddb-dbi", version), null, false)
        .asScala.filter(!_.isDirectory).map(_.getAbsolutePath).filter(_.endsWith("jar"))
  ...
  override def parseWriter(writerData: JValue): Unit = {
    val prefix = writerPrefix("ddb.dbi")
    implicit val formats: DefaultFormats.type = DefaultFormats
    val properties = Properties.properties
    addSparkConf(properties, "spark.transmit.writer", "com.netease.music.da.transfer.ddb.dbi.writer.DDBDBIWriter")
    ...
    addSparkConfIfExist(writerData, "conditions", properties, combineKey(prefix, "condition"))
