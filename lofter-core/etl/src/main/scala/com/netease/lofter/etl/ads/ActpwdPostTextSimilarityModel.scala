...
import org.apache.spark.ml.feature.{CountVectorizer, MinHashLSH, RegexTokenizer}
import org.apache.spark.sql.{SaveMode, SparkSession}
import org.apache.spark.sql.functions.{col, udf}
import org.joda.time.DateTime

object ActpwdPostTextSimilarityModel {
  ...
  def isNotEmptyVector(vect: org.apache.spark.ml.linalg.Vector): Boolean = {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val actpwdPosts =
      s"""
         ...
         |       is_first_level_channel = 1 and
         |		    from_unixtime(cast(startTime/1000 as bigint), 'yyyy-MM-dd') >= '2023-03-01'
         |	group by itemId
         |) a
         |join (
         |	select id, blogid, digest, publishTime,title
         |	from lofter_db_dump.ods_db_post_nd
         |	where type = 1 and valid = 0 and allowview = 0 and isPublished = 1 and citeRootPostid = 0
         |) b on a.itemid = b.id
         |where length(replace(html_text(b.digest), '[图片]', '')) >= 20
         |""".stripMargin

    val sentenceDataFrame = spark.sql(actpwdPosts).cache()
    ...
    val cvModel = new CountVectorizer()
      ...
      .setVocabSize(20000000)
      .setBinary(true)
      .fit(regTokenized)

    val countVectored = cvModel.transform(regTokenized)
    ...
    val actpwdFeature = model.transform(countVectored)

    cvModel.write.overwrite().save(s"/user/da_lofter/warehouse/text_similarity_cv/dt=$date")
    model.write.overwrite().save(s"/user/da_lofter/warehouse/text_similarity_model/dt=$date")
    actpwdFeature.write.mode(SaveMode.Overwrite).parquet(s"/user/da_lofter/warehouse/text_similarity_actpwd_posts/dt=$date")
  }
}
