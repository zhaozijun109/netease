...
object TagDataHandle2Clickhouse {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val query ={
      ...
      s"""
         |INSERT OVERWRITE TABLE lofter_dm.ads_lofter_tags_data_dd PARTITION (dt = '$dt', tag = '${stringToBase64(tag)}')
    ...
    var sourceQuery = s"select dt, '$tag' as tag, dim1, dim2, dim3, dim4, grp, value, userId from lofter_dm.ads_lofter_tags_data_dd where dt = '$dt' and tag = '${stringToBase64(tag)}'"
    if(bitmap){
      destTable = s"$b_line.user_portrait_datas_bitmap"
      val bitmap_sql =
        s"""
           |INSERT OVERWRITE TABLE lofter_dm.ads_lofter_tags_data_bitmap_dd PARTITION (dt, tag)
           |select max(tag_name) as tag_name,
           |dim1,dim2,dim3,dim4,grp,
           |to_bitmap(userId) as bitmap,
           |max(dt) as dt,
           |max(tag) as tag
           |from lofter_dm.ads_lofter_tags_data_dd
           |where dt='$dt' and tag = '${stringToBase64(tag)}'
           |group by dim1,dim2,dim3,dim4,grp
           |""".stripMargin
      println(bitmap_sql)
      spark.sql(bitmap_sql)
      sourceQuery = s"select dt, '$tag' as tag, dim1, dim2, dim3, dim4, grp, bitmap as users_bitmap from lofter_dm.ads_lofter_tags_data_bitmap_dd where dt = '$dt' and tag = '${stringToBase64(tag)}'"
    }
    ...
    for(t <- tag1.split(",")){
      ...
      Seq(datajson).toDF("datajson")
        ...
        .write
        .format("kafka")
//        .option("kafka.bootstrap.servers","lofter-dc-kafka4.service.163.org:9092,lofter-dc-kafka5.service.163.org:9092,lofter-dc-kafka6.service.163.org:9092")
