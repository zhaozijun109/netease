  ...
  def main(args: Array[String]): Unit = {
    ...
    val candidatePosts =
      s"""
         ...
         |        where dt='$date' and createDate = '$date'
         |        group by 1
         |    ) b on a.id = b.postId
         |    join (
         |        select postId
         ...
         |    where d.blogId is null and e.blogId is null
         |) t
    ...
    val cvModel = CountVectorizerModel.load(s"/user/da_lofter/warehouse/gift_post_similarity_cv/dt=$date")
    ...
    val model = MinHashLSHModel.load(s"/user/da_lofter/warehouse/gift_post_similarity_model/dt=$date")
    val giftPostFeatures: Dataset[Row] = spark.read.parquet(s"/user/da_lofter/warehouse/gift_post_similarity_base_posts/dt=$date")
    ...
    spark.sql(s"insert overwrite table lofter_dm.ads_gift_post_similarity_di partition (dt = '$date') select * from result")
  }
}
