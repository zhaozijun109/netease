...
object ActpwdPostTextSimilarity {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val candidatePosts =
     s"""
        |select a.id, substring(html_text(a1.content), 0, 370) as digest, a.blogId
        |from (
        |    select id, blogId
        |    from lofter_db_dump.ods_db_post_nd
        |    where type = 1 and valid = 0 and allowview = 0 and
        |	         isPublished = 1 and citeRootPostid = 0 and
        |		       from_unixtime(cast(publishTime / 1000 AS BIGINT), 'yyyy-MM-dd') BETWEEN '$dayAgo' and '$date'
        |) a
        |join lofter_db_dump.ods_db_text_post_nd a1 on a.id = a1.id
        |left join (
        ...
        |  union all
        |    select userId as blogid from lofter.dws_par_creator_level_scoring_dd where dt = '$dayAgo' and level in ('S','A','B','C')
    ...
    val cvModel = CountVectorizerModel.load(s"/user/da_lofter/warehouse/text_similarity_cv/dt=$date")
    ...
    val model = MinHashLSHModel.load(s"/user/da_lofter/warehouse/text_similarity_model/dt=$date")
    val actpwdFeature: Dataset[Row] = spark.read.parquet(s"/user/da_lofter/warehouse/text_similarity_actpwd_posts/dt=$date")
    val candidateFeature: Dataset[Row] = model.transform(candidatePostRawFeatures)

    val threshold = 0.8
    val distThreshold = 0.3

    val simiDistance = model.approxSimilarityJoin(candidateFeature, actpwdFeature, threshold)
      .select(col("datasetA.id").alias("postId"),
        col("datasetA.blogId").alias(s"blogId"),
        col("datasetA.sentence").alias(s"post_digest"),
        col("datasetB.id").alias("actpwd_post_id"),
        col("datasetB.sentence").alias(s"actpwd_post_digest"),
        col("distCol")
      ).where(s"datasetA.blogId != datasetB.blogId and distCol < $distThreshold")

    simiDistance.createOrReplaceTempView("result")

    spark.sql(s"insert overwrite table lofter_dm.ads_risk_actpwd_similar_posts_di partition (dt = '$date') select * from result")
  }

  case class SimilarPostEntry(postId: Long, blogId: Long, post_digest: String, actpwd_post_id: Long, actpwd_post_digest: String, distCol: Double)
