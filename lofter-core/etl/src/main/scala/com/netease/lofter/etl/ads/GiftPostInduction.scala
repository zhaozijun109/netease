  ...
  def main(args: Array[String]): Unit = {
    ...
    val candidatePosts =
      s"""
         ...
         |             from_unixtime(cast(publishTime / 1000 AS BIGINT), 'yyyy-MM-dd') = '$date'
         |    ) a
         |    left join (
         |        select postId
    ...
    spark.sql(s"insert overwrite table lofter_dm.ads_gift_post_induction_di partition (dt = '$date') select * from result")
  }
}
