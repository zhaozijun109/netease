...
object CreatorCenterDataWeekly {
  def main(args: Array[String]): Unit = {
    ...
    val isWeekEnd = DateTime.parse(dt).getDayOfWeek == 7

    if (isWeekEnd) {
      val weekDt = DateTime.parse(dt).withDayOfWeek(1).toString("yyyy-MM-dd")
      val weekStart = weekDt
      val weekEnd = DateTime.parse(dt).withDayOfWeek(7).toString("yyyy-MM-dd")
      ...
      val creatorGeneral =
        s"""
           ...
           |               count(1) as hot,
           |               sum(if(post_publish_date >= '$weekStart' and post_publish_date <= '$weekEnd' , 1, 0)) newPostHot
      ...
      spark.sql(creatorSql)
        .withColumn("dt", lit(weekDt))
        .repartition(10)
        .write.mode(SaveMode.Overwrite)
        .insertInto("lofter_dm.ads_par_creator_rank_wd")
    }
