...
object GiftPostTextSimilarityModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val actpwdPosts =
      s"""
         ...
         |    from (
         |        select id, blogId, publishTime, html_text(digest) as digest
         ...
         |             datediff('$date', from_unixtime(cast(publishTime / 1000 AS BIGINT), 'yyyy-MM-dd')) < 90
         |    ) a
         |    join (
         |        select postId
         |        from lofter.dim_gift_post_return_dd
         |        where dt='$dayAgo' and status = 1 and auditStatus = 1 and
         |              createDate <= '$dayAgo'
         |        group by 1
         |      union
         |        select postId
         |        from lofter.dws_post_interaction_dd
         |        where dt = '$dayAgo' and hot >= 1000
         |    ) b on a.id = b.postId
         |    join (
         |        select postId
         |        from lofter.dws_post_base_stats_dd
         |        where dt='$date' and (hdpv > 10 or exposureuv > 10)
         |    ) c on a.id = c.postId
         |    left join (
         |        select blogId from lofter.dim_miniprogram_post_dd where dt='$date' group by 1
         |    ) d on a.blogId = d.blogId
         |    where d.blogId is null
         |) t
    ...
    cvModel.write.overwrite().save(s"/user/da_lofter/warehouse/gift_post_similarity_cv/dt=$date")
    model.write.overwrite().save(s"/user/da_lofter/warehouse/gift_post_similarity_model/dt=$date")
    actpwdFeature.write.mode("overwrite").parquet(s"/user/da_lofter/warehouse/gift_post_similarity_base_posts/dt=$date")
  }
}
