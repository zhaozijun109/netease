...
object ActpwdPostTextSimilarityHourly {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val candidatePosts =
      s"""
         |select a.*
         |from (
         |    select id, html_text(digest) as digest, blogId
         |    from (
         ...
         |		       from_unixtime(cast(publishTime / 1000 AS BIGINT), 'yyyy-MM-dd') = '$today' and
    ...
    val cvModel = CountVectorizerModel.load(getLatest("/user/da_lofter/warehouse/text_similarity_cv"))
    ...
    val model = MinHashLSHModel.load(getLatest("/user/da_lofter/warehouse/text_similarity_model"))
    val actpwdFeature: Dataset[Row] = spark.read.parquet(getLatest("/user/da_lofter/warehouse/text_similarity_actpwd_posts"))
    ...
    spark.sql(s"insert overwrite table lofter_dm.ads_risk_actpwd_similar_posts_hi partition (dt = '$today', hour = $hour) select * from result")
  }
