 ...
 *    --key: kafka partition column
 *    --order: kafka partition key values order column
 */
object Hive2Kafka {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val filterTopic = Set("LOFTER.ReturnGift.Auto.Present", "lofter.push.collection.update",
      "lofter.creator.weekReport", "LOFTER.FINIANCE.STORE_READ_RATIO", "lofter.push.rec.content",
      "LOFTER.POPUP.high_recom_posts", "rec_bdms_premium_content", "rec_bdms_guide_ip_tags", "AD.RESOURCE_MAISUI.ADPOSTPOOL",
      "LOFTER.ASO.PAYFORACTIVE"
    )
    ...
    val order = pargs.optional("order")
    ...
    if(userName.isDefined && password.isDefined) {
      ...
      if(order.isDefined) {
        dataFinal.repartition(parallel, col(key.get))
          .sortWithinPartitions(col(key.get), col(order.get))
