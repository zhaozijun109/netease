...
import redis.clients.jedis.{Connection, HostAndPort, JedisCluster}
 ...
 *    --enableTtl: enable ttl for redis key
...
object Hive2Redis {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cluster = pargs.optional("cluster").getOrElse(redisConfig.REDIS_URL)
    ...
    // val username = pargs.optional("username").getOrElse(redisConfig.REDIS_USERNAME)
    val password = pargs.optional("password").getOrElse(redisConfig.REDIS_PASSWORD)

    val currentTimeMillis = System.currentTimeMillis()
    val (timeValue, timeUnits): (Option[Int], Option[String]) = if (pargs.boolean("enableTtl")) {
    ...
    val spark = SparkSession.builder()
      .appName("Lofter Hive2Redis Task")
      .enableHiveSupport()
    ...
    df.repartition(parallel)
      .foreachPartition(
        (p: Iterator[Row]) => {
          val jedisCluster = createJedisClient(redisHostAndPorts, password)
          // Single thread
  ...
  private def createJedisClient(hostAndPortSet: util.Set[HostAndPort], password: String): JedisCluster = {
    val poolConfig = new GenericObjectPoolConfig[Connection]
    poolConfig.setMaxTotal(30)
    poolConfig.setMaxIdle(30)
    poolConfig.setMinIdle(1)
    new JedisCluster(hostAndPortSet, 10000, 10000, 3, password, poolConfig)
  }
  ...
  private def createSingleConsumer(data: Seq[Row], jedisCluster: JedisCluster, currentTimeMillis: Long,
    ...
    data.foreach {
      row => writeToRedis(row, jedisCluster, currentTimeMillis, timeValue, timeUnits)
    }
  }
  ...
  private def writeToRedis(row: Row, jedisCluster: JedisCluster, currentTimeMillis: Long,
                           timeValue: Option[Int], timeUnits: Option[String]): Unit = {
    if (row.getAs("key") == null) {
      throw new Exception("The key cannot be null!")
    }
    val key = row.getAs("key").toString
    val value = if (row.getAs("value") == null) "da_null" else row.getAs("value").toString
