...
object Hive2ES {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val filterIndex = Set("lofter_miniprogram_list", "lofter_video_blog_bind_monthly", "lofter_video_blog_bind_monthly_flag",
      "lofter_gift_return_hot_list", "lofter_batch_grain_hot_list", "lofter_author_data_user_weekly", "lofter_ads_column_",
      "lofter_post_group_pack_summary", "lofter_post_group_post_detail", "lofter_post_group_pack_posts", "lofter_tag_example_pay_post"
    )
    ...
    val esRequest: IndicesAliasesRequest = new IndicesAliasesRequest()
    ...
    if (atom) {
      df.repartition(parallel).saveToEs(s"$destIndex/_doc", writeEsConfig)
      ...
      if(full) { // replace old partitions with the new partition through alias replacing
        finalAliases foreach { alias =>
          val removeAliasAction: AliasActions = new AliasActions(AliasActions.Type.REMOVE).index(destGlobs).alias(alias)
          val addAliasAction: AliasActions = new AliasActions(AliasActions.Type.ADD).index(destIndex).alias(alias)
          esRequest.addAliasAction(removeAliasAction).addAliasAction(addAliasAction)
        }
      } else { // add partition alias
        finalAliases foreach { alias =>
          val addAliasAction: AliasActions = new AliasActions(AliasActions.Type.ADD).index(destIndex).alias(alias)
          esRequest.addAliasAction(addAliasAction)
        }
      }

      try {
        val response = client.indices().updateAliases(esRequest, RequestOptions.DEFAULT)

        if (!response.isAcknowledged) { System.exit(1) }
      } catch {
