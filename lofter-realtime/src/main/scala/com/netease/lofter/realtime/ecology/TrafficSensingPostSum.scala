package com.netease.lofter.realtime.ecology

import com.google.common.util.concurrent.MoreExecutors
import com.netease.lofter.realtime.common.{esConfig, kafkaConfig}
import com.netease.wm.hubble.avro.{RecItemEvent, TrafficSensingItemFeature}
import com.netease.wm.hubble.common.avro.binary.{AvroBinaryDeserSchema, AvroBinarySerSchema}
import org.apache.flink.api.common.eventtime.WatermarkStrategy
import org.apache.flink.connector.base.DeliveryGuarantee
import org.apache.flink.connector.kafka.sink.{KafkaRecordSerializationSchema, KafkaSink}
import org.apache.flink.connector.kafka.source.KafkaSource
import org.apache.flink.connector.kafka.source.enumerator.initializer.OffsetsInitializer
import org.apache.flink.streaming.api.environment.CheckpointConfig.ExternalizedCheckpointCleanup
import org.apache.flink.streaming.api.scala._
import org.apache.flink.streaming.api.scala.async.{AsyncFunction, ResultFuture}
import org.apache.flink.table.api.bridge.scala.{StreamTableEnvironment, tableConversions}
import org.apache.flink.types.RowKind
import org.apache.kafka.clients.consumer.OffsetResetStrategy
import org.elasticsearch.action.ActionListener
import org.elasticsearch.action.search.{SearchRequest, SearchResponse}
import org.elasticsearch.client.{RestClient, RestHighLevelClient}
import org.elasticsearch.index.query.QueryBuilders
import org.elasticsearch.search.builder.SearchSourceBuilder
import org.joda.time.DateTime
import org.slf4j.{Logger, LoggerFactory}

import java.time.Duration
import scala.concurrent.duration.DurationInt
import scala.concurrent.{ExecutionContext, Future, Promise}
import scala.util.control.NonFatal

object TrafficSensingPostSum {
  case class Event(itemId:String, itemType: String, userId: Long, time: Long, action: Int, action_value: Int,
                   interaction_type: Int, publishTime: Long, blogLevel: String, ip: String, isInRecommendPool: Int,
                   sourceScene: String)

  def main(args: Array[String]): Unit = {
    val env = StreamExecutionEnvironment.getExecutionEnvironment
    val tableEnv = StreamTableEnvironment.create(env)

    env.enableCheckpointing(300000)
    env.getCheckpointConfig.enableExternalizedCheckpoints(ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION)
    env.getConfig.enableObjectReuse()

    val startTimeStamp = new DateTime().withTimeAtStartOfDay().getMillis

    val eventDetailSource = KafkaSource.builder[RecItemEvent]()
      .setBootstrapServers(kafkaConfig.GY_BOOTSTRAP_SERVERS)
      .setTopics("rec.item.detail")
      .setGroupId("traffic_sensing_post_sum")
      .setStartingOffsets(OffsetsInitializer.committedOffsets(OffsetResetStrategy.LATEST))
      .setValueOnlyDeserializer(new AvroBinaryDeserSchema[RecItemEvent])
      .build()

    val aggregateSink = KafkaSink.builder[TrafficSensingItemFeature]()
      .setDeliverGuarantee(DeliveryGuarantee.AT_LEAST_ONCE)
      .setBootstrapServers(kafkaConfig.GY_BOOTSTRAP_SERVERS)
      .setRecordSerializer(
        KafkaRecordSerializationSchema.builder()
          .setTopic("rec.item.feature")
          .setValueSerializationSchema(new AvroBinarySerSchema[TrafficSensingItemFeature])
          .build()
      ).build()

    val eventDetail: DataStream[RecItemEvent] = env.fromSource(eventDetailSource, WatermarkStrategy.noWatermarks(), "rec-item-event-detail")
      .uid("traffic-sensing-input")
      .disableChaining()
      .filter(e => e.itemId.contains("_") && e.repeat.getOrElse(0L) == 0L && (e.action == 1 || e.action == 5))

    val newPostEventDetail = AsyncDataStream.unorderedWait(eventDetail, new ResolveBlogLevelAndPostIp, 30, concurrent.duration.SECONDS, 100)

    tableEnv.createTemporaryView("new_post_event", newPostEventDetail)

    val configuration = tableEnv.getConfig.getConfiguration
    configuration.setString("table.exec.mini-batch.enabled", "true")
    configuration.setString("table.exec.mini-batch.allow-latency", "30 s")
    configuration.setString("table.exec.mini-batch.size", "10000")

    tableEnv.getConfig.setIdleStateRetention(Duration.ofDays(8))

    val featureSql =
      """
        |select itemId, itemType, publishTime, blogLevel, ip,
        |       count(1) filter (where action = 1) as exposure,
        |       sum(action_value) filter (where action = 5 and interaction_type = 3 ) as recommend,
        |       sum(action_value) filter (where action = 5 and interaction_type = 6 ) as share,
        |       sum(action_value) filter (where action = 5 and interaction_type >= 1 and interaction_type <= 4) as hot,
        |       max(isInRecommendPool) as isInRecommendPool,
        |       sum(action_value) filter (where action = 5 and interaction_type = 5) as response,
        |       sum(action_value) filter (where action = 3 and sourceScene = 'collection') as collectionBrowse,
        |       sum(action_value) filter (where action = 1 and sourceScene = 'recommend') as recommendExposure,
        |       count(1) filter (where action = 2) as click
        |from new_post_event
        |group by itemId, itemType, publishTime, blogLevel, ip
        |""".stripMargin

    tableEnv.sqlQuery(featureSql)
      .toChangelogStream
      .filter(row => row.getKind == RowKind.INSERT || row.getKind == RowKind.UPDATE_AFTER)
      .map{ x =>
        TrafficSensingItemFeature(
          x.getFieldAs("itemId"), x.getFieldAs("itemType"), x.getFieldAs("publishTime"),
          x.getFieldAs("blogLevel"), x.getFieldAs("ip"), x.getFieldAs[Long]("exposure"), x.getFieldAs[Int]("hot"),
          x.getFieldAs[Int]("recommend"), x.getFieldAs[Int]("share"), x.getFieldAs[Int]("isInRecommendPool"),
          Option(x.getFieldAs[Int]("response")), Option(x.getFieldAs[Int]("collectionBrowse")),
          Option(x.getFieldAs[Int]("recommendExposure")), Option(x.getFieldAs[Long]("click")))
      }.sinkTo(aggregateSink).uid("traffic-sensing-item-feature-sink")

    env.execute("traffic sensing post sum v3")
  }

  implicit lazy val executor: ExecutionContext = ExecutionContext.fromExecutor(MoreExecutors.directExecutor())

  class ResolveBlogLevelAndPostIp extends AsyncFunction[RecItemEvent, Event] {

    @transient private lazy val postCache = PostCache

    override def asyncInvoke(e: RecItemEvent, resultFuture: ResultFuture[Event]): Unit = {
      val postIdHex = e.itemId.split("_").tail.head
      try {
        val postId = java.lang.Long.parseLong(postIdHex, 16)
        PostCache.getPostIpMeta(postId).foreach { meta =>
          val sevenDaysAgo = DateTime.now().minusDays(7).getMillis
          if(meta.publishTime > sevenDaysAgo && meta.publishTime >= 1768995359000L) {
            val isInRecommendPool = if(meta.isInRecommendPool) 1 else 0
            val result = Event(
              e.itemId, e.itemType, e.userId, e.time, e.action, e.action_value,
              e.interaction_type.getOrElse(0), meta.publishTime, meta.blogLevel, meta.ip, isInRecommendPool,
              e.source_scene.getOrElse("")
            )
            resultFuture complete Seq(result)
          } else {
            resultFuture complete Seq.empty// ignore old post
          }
        }
      } catch {
        case NonFatal(e) =>
          e.printStackTrace()
          resultFuture complete Seq.empty
      }
    }
  }

  case class Post(postId: Long, publishTime: Long, isInRecommendPool: Boolean, blogLevel: String, ip: String)

  private class PostCache{}
  private object PostCache {
    import com.github.blemale.scaffeine.{AsyncLoadingCache, Scaffeine}

    val LOG: Logger = LoggerFactory.getLogger(classOf[Post])

    lazy val client: RestHighLevelClient = new RestHighLevelClient(RestClient.builder(esConfig.ES_HOSTS:_*))

    lazy val postDimCache: AsyncLoadingCache[Long, Post] =
      Scaffeine()
        .maximumSize(2000000)
        .expireAfterWrite(30.minutes)
        .buildAsyncFuture(
          (postId: Long) => fetchPostIpMeta(Seq(postId)) { m => m.getOrElse(postId, getDefaultPost(postId))},
          allLoader = Option((posts: Iterable[Long]) => fetchPostIpMeta(posts.toSeq)(identity))
        )

    private def fetchPostIpMeta[T](posts: Seq[Long])(mapFunc: Map[Long, Post] => T): Future[T] = {
      val searchRequest = new SearchRequest("lofter_dim_ip_new_post")

      val conditions = QueryBuilders.boolQuery()

      posts.foreach { p =>
        conditions.should(QueryBuilders.termQuery("postId", p))
      }

      val sourceBuilder = new SearchSourceBuilder()
        .query(conditions)
        .fetchSource(false)
        .docValueField("postId", "use_field_mapping")
        .docValueField("publishTime", "use_field_mapping")
        .docValueField("isInRecommendPool", "use_field_mapping")
        .docValueField("blogLevel", "use_field_mapping")
        .docValueField("ip", "use_field_mapping")

      searchRequest.source(sourceBuilder)
      val result = Promise[T]()

      client.searchAsync(searchRequest, new ActionListener[SearchResponse] {

        override def onFailure(e: Exception): Unit = result.failure(e)

        override def onResponse(searchResponse: SearchResponse): Unit = {
          LOG.debug("get es response: {}", searchResponse)
          try {
            val hits = searchResponse.getHits.getHits()
            val queryMap = hits.map { hit =>
              val postId = hit.field("postId").getValue[Number].longValue()
              val publishTime = hit.field("publishTime").getValue[Number].longValue()
              val isInRecommendPool = hit.field("isInRecommendPool").getValue[Number].longValue()
              val blogLevel = hit.field("blogLevel").getValue[String]
              val ip = hit.field("ip").getValue[String]
              postId -> Post(postId, publishTime, isInRecommendPool > 0, blogLevel, ip)
            }.toMap

            if(queryMap.size < posts.size) {
              val permalinkMap: Map[Long, Post] = (posts map { p: Long =>  p -> queryMap.getOrElse(p, getDefaultPost(p))}).toMap
              result success mapFunc(permalinkMap)
            } else {
              result success mapFunc(queryMap)
            }
          } catch {
            case NonFatal(e) =>
              LOG.warn("query es error", e)
              result failure(e)
          }
        }
      })

      result.future
    }

    def getDefaultPost(postId: Long): Post = Post(postId, 0L, false, "", "")

    def getPostIpMeta(posts: Seq[Long]): Future[Map[Long, Post]] = {
      postDimCache.getAll(posts)
    }

    def getPostIpMeta(postId: Long): Future[Post] = {
      postDimCache.get(postId)
    }
  }
}