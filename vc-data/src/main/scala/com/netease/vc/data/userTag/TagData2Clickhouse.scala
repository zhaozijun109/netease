...
//      .master("local[*]")
    ...
    val destTable = pargs.required("cktable")
    val tags = pargs.required("tags")
    val b_line = pargs.required("b_line")
    val tag_type = pargs.required("tag_type")

    tags.split(",").foreach(tag => {
      val sourceQuery = s"select dt, tag, value1, value2, value3, value4, value5, userId from vc.ads_vc_tag_attributes_dd where dt = '${dt}' and tag = '${tag}'"
      ...
      Seq((datajson,1)).toDF("datajson","key")
        .selectExpr("CAST(key as string) as key","CAST(datajson as string) as value")
        .write
        .format("kafka")
