...
object Hive2ClickhouseAutoSync {
  ...
  private val INC_POSTFIX_PATTERN = """ads_.*_(di|wi|mi|dd|wd|md|[0-9]+d)$""".r
  private val DF = DateTimeFormatter.ofPattern("yyyy-MM-dd")

  val history_tabls = Set("ads_pve_app_chat_retain_data_di","ads_pve_app_chat_base_data_di","ads_vc_ad_cvr_di","ads_vc_ad_cvr_new_di",
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (System.currentTimeMillis() - startTime < 440 * 1000 && localDateTime.getHour > 4) {
      ...
      val globTables = (importTableList ++ clickhouseConfig.historyLoadJobs)
        .filter {
          case tn @ INC_POSTFIX_PATTERN(_) if (history_tabls.contains(tn) || tn.contains("_report_") || tn.contains("_easy_fetch_"))=> true
          case _ => false
        }
