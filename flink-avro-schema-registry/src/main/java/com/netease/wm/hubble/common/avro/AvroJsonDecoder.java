...
/**
 * Modified JsonDecoder, refer AvroJsonEncoder
 */
public class AvroJsonDecoder extends ParsingDecoder implements Parser.ActionHandler {
    ...
    private AvroJsonDecoder(Symbol root, InputStream in) throws IOException {
        super(root);
        configure(in);
    }

    private AvroJsonDecoder(Symbol root, String in) throws IOException {
        super(root);
        configure(in);
    }

    public AvroJsonDecoder(Schema schema, InputStream in) throws IOException {
        this(getSymbol(schema), in);
    }

    public AvroJsonDecoder(Schema schema, String in) throws IOException {
        this(getSymbol(schema), in);
    }
     ...
     */
    public AvroJsonDecoder configure(InputStream in) throws IOException {
     ...
     * @return this JsonDecoder
     */
    public AvroJsonDecoder configure(String in) throws IOException {
    ...
    public int readIndex() throws IOException {
        ...
        if (in.getCurrentToken() == JsonToken.VALUE_NULL) {
            label = "null";
        } else if (AvroJsonEncoder.isNullableSingle(a)) {
            label = AvroJsonEncoder.getNullableSingle(a);
