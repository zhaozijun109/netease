package com.netease.wm.hubble.common.avro;
...
/**
 *
 * copied from org.apache.avro.io.BinaryDecoder, the main goal is to expose byte array constructor directly.
 * This class use little avro internal api, should be stably compatible with new avro
 *
 */
public class InnerBinaryDecoder extends Decoder {
    ...
    public InnerBinaryDecoder() {
    }

    public InnerBinaryDecoder(InputStream in, int bufferSize) {
        super();
        configure(in, bufferSize);
    }

    public InnerBinaryDecoder(byte[] data, int offset, int length) {
        super();
        configure(data, offset, length);
    }

    InnerBinaryDecoder configure(InputStream in, int bufferSize) {
        configureSource(bufferSize, new InputStreamByteSource(in));
        return this;
    }

    InnerBinaryDecoder configure(byte[] data, int offset, int length) {
    ...
    static class BufferAccessor {
        private final InnerBinaryDecoder decoder;
        private byte[] buf;
        private int pos;
        private int limit;
        boolean detached = false;

        private BufferAccessor(InnerBinaryDecoder decoder) {
    ...
    abstract static class ByteSource extends InputStream {
        ...
        protected void attach(int bufferSize, InnerBinaryDecoder decoder) {
