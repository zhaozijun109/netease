...
import org.apache.spark.ml.util_.{Utils => MLUtils, _}
import org.apache.spark.ml.{Estimator, Model}
import org.apache.spark.mllib.feature_
 ...
 */
trait Word2VecBase extends Params
  ...
  setDefault(vectorSize -> 100, windowSize -> 5, numPartitions -> 1, minCount -> 5,
    maxSentenceLength -> 1000, stepSize -> 0.025, maxIter -> 1, computeLoss -> true, shuffle -> false, sliceSave -> false)
...
final class Word2Vec @Since("1.4.0")(
  ...
  override def fit(dataset: Dataset[_]): Word2VecModel = {
    ...
    val wordVectors = new feature_.Word2Vec()
...
class Word2VecModel(
                     @Since("1.4.0") override val uid: String,
                     @transient private val wordVectors: feature_.Word2VecModel)
  ...
  @transient lazy val getVectors: DataFrame = {
    ...
    MLUtils.wordVecToRDD(spark, wordVectors).map {
      case (word, vec) =>
        (word, Vectors.dense(vec.map(_.toDouble)))
    }.toDF("word", "vector")
  }
...
object Word2VecModel extends MLReadable[Word2VecModel] {
  ...
  class Word2VecModelWriter(instance: Word2VecModel) extends MLWriter {
    ...
    override protected def saveImpl(path: String): Unit = {
      ...
      if (sliceSave) {
        MLUtils.wordVecToDF(spark, instance.wordVectors)
          .write
          .parquet(dataPath)
      } else {
