...
package org.apache.spark.ml.linalg_
...
object BLAS extends Serializable {
  ...
  private def syr(alpha: Double, x: DenseVector, A: DenseMatrix): Unit = {
    ...
    val update = classOf[DenseMatrix].getDeclaredMethod("update", classOf[Int], classOf[Int], classOf[Double])
    ...
    while (i < mA) {
      ...
      while (j < nA) {
//        A(j, i) = A(i, j)
        update.invoke(A, j.asInstanceOf[Object], i.asInstanceOf[Object], A(i, j).asInstanceOf[Object])
        j += 1
      }
      i += 1
    }
