...
package org.apache.spark.mllib.feature_
...
import org.apache.spark.ml.util_.{Utils => MLUtils}
import org.apache.spark.mllib.linalg.{Vector, Vectors}
import org.apache.spark.mllib.util.{Loader, Saveable}
import org.apache.spark.rdd._
import org.apache.spark.sql.SparkSession
import org.apache.spark.util.random.XORShiftRandom
import org.apache.spark.util.{BoundedPriorityQueue, DoubleAccumulator, Utils}
...
class Word2Vec extends Serializable with Logging {
  ...
  private var computeLoss = true
  private var shuffle = false
  private var sliceSave = false
  ...
  def setComputeLoss(computeLoss: Boolean): this.type = {
    this.computeLoss = computeLoss
    this
  }

  def setShuffle(shuffle: Boolean): this.type = {
    this.shuffle = shuffle
    this
  }

  def setSliceSave(sliceSave: Boolean): this.type = {
    this.sliceSave = sliceSave
    this
  }
  ...
  def fit[S <: Iterable[String]](dataset: RDD[S]): Word2VecModel = {
    ...
    val logExpTable = expTable.map(f => Math.log(f).toFloat)
    val bcLogExpTable = sc.broadcast(logExpTable)
    val bcVocab = sc.broadcast(vocab)
    val bcVocabHash = sc.broadcast(vocabHash)
    try {
      doFit(dataset, sc, bcExpTable, bcLogExpTable, bcVocab, bcVocabHash)
    } finally {
      bcExpTable.destroy()
      bcLogExpTable.destroy()
      bcVocab.destroy()
      bcVocabHash.destroy()
    }
  }

  def doShuffle(rnd: Random, values: Array[Int]): Unit = {
  ...
  private def doFit[S <: Iterable[String]](dataset: RDD[S], sc: SparkContext,
                                           ...
                                           logExpTable: Broadcast[Array[Float]],
                  ...
                  if (c >= 0 && c < sentence.length) {
                    ...
                    while (d < bcVocab.value(word).codeLen) {
                      ...
                      if (f > -MAX_EXP && f < MAX_EXP) {
                        if (computeLoss) {
                          val sgn = if (bcVocab.value(word).code(d) == 1) -1 else 1
                          val lprob = -1 * sgn * f
                          if (lprob > -MAX_EXP && lprob < MAX_EXP) {
                            val ind = ((lprob + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2.0)).toInt
                            loss -= logExpTable.value(ind)
                          }
                        }
    ...
    new Word2VecModel(wordArray.zipWithIndex.toMap, syn0Global).setSliceSave(sliceSave)
  }
 ...
 */
@Since("1.1.0")
class Word2VecModel(val wordIndex: Map[String, Int],
  ...
  def save(sc: SparkContext, path: String): Unit = {
    if (sliceSave) {
      Word2VecModel.SaveLoadV1_0.save(sc, path, this)
    } else {
...
object Word2VecModel extends Loader[Word2VecModel] {
  ...
  private object SaveLoadV1_0 {
    ...
    def save(sc: SparkContext, path: String, model: Word2VecModel): Unit = {
      val spark = SparkSession.builder().sparkContext(sc).getOrCreate()

      val numWords = model.wordIndex.size
      val vectorSize = model.wordVectors.length / numWords
      val metadata = compact(render(
        ("class" -> classNameV1_0) ~ ("version" -> formatVersionV1_0) ~
          ("vectorSize" -> vectorSize) ~ ("numWords" -> numWords)))
      sc.parallelize(Seq(metadata), 1).saveAsTextFile(Loader.metadataPath(path))
      MLUtils.wordVecToDF(spark, model)
