...
import com.linkedin.spark.shaded.org.tensorflow.example._
 ...
 */
class TFRecordSerializer(dataSchema: StructType) extends Serializable {

  private val featureConverters = dataSchema.map(_.dataType).map(newFeatureConverter).toArray

  private val encoder: ExpressionEncoder[Row] = RowEncoder.apply(dataSchema).resolveAndBind()

  private val serializer = encoder.createSerializer()

  def serializeExample(row: Row): Example = {
    val internalRow = serializer(row)
    serializeExample(internalRow)
  }
  ...
  def serializeSequenceExample(row: Row): SequenceExample = {
    val internalRow = serializer(row)
    serializeSequenceExample(internalRow)
  }
