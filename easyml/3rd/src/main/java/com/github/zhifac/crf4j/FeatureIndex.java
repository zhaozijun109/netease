...
public abstract class FeatureIndex {
    ...
    public String getIndex(Example example, String[] idxStr, int cur, TaggerImpl tagger) {
        int row = Integer.valueOf(idxStr[0]);
        int col = Integer.valueOf(idxStr[1]);
        int pos = row + cur;
        if (row < -EOS.length || row > EOS.length || col < 0 || col >= tagger.xsize()) {
        ...
        if (pos < 0) {
            return BOS[-pos - 1];
        } else if (pos >= tagger.size(example)) {
            return EOS[pos - tagger.size(example)];
        } else {
            return tagger.x(example, pos, col);
        }
    }
    ...
    public String applyRule(Example example, String str, int cur, TaggerImpl tagger) {
        StringBuilder sb = new StringBuilder();
        for (String tmp : str.split("%x", -1)) {
            if (tmp.startsWith("U") || tmp.startsWith("B")) {
                sb.append(tmp);
                ...
                String[] tuple = tmp.split("]");
                String[] idx = tuple[0].replace("[", "").split(",");
                String r = getIndex(example, idx, cur, tagger);
                if (r != null) {
                    sb.append(r);
                }
                if (tuple.length > 1) {
    ...
    private boolean buildFeatureFromTempl(Example example, List<Integer> feature, List<String> templs, int curPos, TaggerImpl tagger) {
        for (String tmpl : templs) {
            String featureID = applyRule(example, tmpl, curPos, tagger);
            if (featureID == null || featureID.length() == 0) {
                System.err.println("format error");
                return false;
            }
    ...
    public boolean buildFeatures(Example example, TaggerImpl tagger) {
        List<Integer> feature = new ArrayList<Integer>();
        List<List<Integer>> featureCache = tagger.getFeatureCache_(example);
        tagger.setFeature_id_(example, featureCache.size());

        for (int cur = 0; cur < tagger.size(example); cur++) {
            if (!buildFeatureFromTempl(example, feature, unigramTempls_, cur, tagger)) {
                return false;
            }
            feature.add(-1);
            featureCache.add(feature);
        ...
        for (int cur = 1; cur < tagger.size(example); cur++) {
            if (!buildFeatureFromTempl(example, feature, bigramTempls_, cur, tagger)) {
                return false;
            }
            feature.add(-1);
            featureCache.add(feature);
    ...
    public void rebuildFeatures(Example example, TaggerImpl tagger) {
        int fid = tagger.getFeature_id_(example);
        List<List<Integer>> featureCache = tagger.getFeatureCache_(example);
        for (int cur = 0; cur < tagger.size(example); cur++) {
            List<Integer> f = featureCache.get(fid++);
            for (int i = 0; i < y_.size(); i++) {
                ...
                n.y = i;
                n.fVector = f;
                tagger.set_node(example, n, cur, i);
            }
        }
        for (int cur = 1; cur < tagger.size(example); cur++) {
            List<Integer> f = featureCache.get(fid++);
            for (int j = 0; j < y_.size(); j++) {
                for (int i = 0; i < y_.size(); i++) {
                    Path p = new Path();
                    p.clear();
                    p.add(tagger.node(example, cur - 1, j), tagger.node(example, cur, i));
                    p.fvector = f;
                }
            }
        }
