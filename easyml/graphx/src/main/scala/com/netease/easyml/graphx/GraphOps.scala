 ...
 */
object GraphOps {
  def setupAlias(nodeWeights: Array[(Int, Double)]): (Array[Int], Array[Double]) = {
    ...
    nodeWeights.zipWithIndex.foreach { case ((nodeId, weight), i) =>
      Q(i) = K * weight / sum
      if (Q(i) < 1.0) smaller.append(i) else larger.append(i)
    }
  ...
  def setupEdgeAlias(p: Double = 1.0, q: Double = 1.0)(srcId: Int, srcNeighbors: Array[(Int, Double)], dstNeighbors: Array[(Int, Double)]): (Array[Int], Array[Double]) = {
  ...
  lazy val createUndirectedEdge = (srcId: Int, dstId: Int, weight: Double) => {
    Array(
  ...
  lazy val createDirectedEdge = (srcId: Int, dstId: Int, weight: Double) => {
    Array(
      (srcId, Array((dstId, weight)))
    )
  }
