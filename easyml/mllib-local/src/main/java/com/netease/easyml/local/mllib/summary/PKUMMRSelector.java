 ...
 */
@AllArgsConstructor
public class PKUMMRSelector implements Selector {
    private static final double PARA = 0.7;
    ...
    public PKUMMRSelector(double maxLen) {
        this(maxLen, BETA);
    }

    public PKUMMRSelector(double maxLen, double beta) {
        this(maxLen, beta, PARA, true);
    }
    ...
    public List<Integer> select(List<List<String>> tokens, double[][] similarity, Map<Integer, Double> ranks) {
        ...
        while (len < maxLen) {
            ...
            for (int i : ranks.keySet()) {
                ...
                for (int j : summaryId) {
                    double tmp = ranks.get(i) - similarity[i][j] * ranks.get(j) * para;
                    if (tmp < tmpscore)
                        tmpscore = tmp;
                }
