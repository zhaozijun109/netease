 ...
 */
public class BowFeaturizer implements IFeaturizer<String, List<Tuple<Integer, Integer>>> {
    ...
    @Override
    public List<Tuple<Integer, Integer>> getFeatures(String text) {
        Counter<String> counter = new Counter<>();
        for (char ch : text.toCharArray())
            counter.add(String.valueOf(ch));
        return counter.getCounter().entrySet().stream().map(it ->
