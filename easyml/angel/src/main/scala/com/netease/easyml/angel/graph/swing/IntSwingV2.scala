 ...
 */
class IntSwingV2(override val uid: String) extends Transformer with SwingParams {
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val (minUserId, maxUserId, numUsers) = u2isDf.select(min(col(u)), max(col(u)), count("*"))
      .rdd.map(row => (row.getInt(0), row.getInt(1), row.getLong(2))).first()
    ...
    psModel.init()
    u2isDf.rdd
      .map(row => (row.getInt(0), row.getSeq[Int](1).toArray))
      .mapPartitions {
        iter =>
    ...
    val retRDD = uuDf.rdd.map(row => (row.getInt(0), row.getInt(1)))
      .mapPartitionsWithIndex {
        case (partId, iter) =>
          IntSwingV2.runNeighborPartition(partId, iter, psModel, pullBatchSize,
            alpha, eps, power
          )
      }.filter(_._3 > 0)
...
object IntSwingV2 {
  ...
  def runNeighborPartition(partId: Int, iter: Iterator[(Int, Int)],
  ...
  def swing(uItems: Array[Int], vItems: Array[Int], alpha: Float, eps: Float, power: Float): Array[(Int, Int, Float)] = {
