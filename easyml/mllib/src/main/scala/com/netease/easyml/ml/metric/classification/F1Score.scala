 ...
 */
@Register(alias = Array("f1", "f1_score", "f1-score"))
class F1Score(override val uid: String) extends Metric
  with FScoreBaseParams with DefaultParamsWritable {
  def this() = this(Identifiable.randomUID("f1"))

  override lazy val shortName: String = "f1-score"
  ...
  override def evaluateJson(dataset: Dataset[_]): Params = {
    ...
    val fscore = param.get(score.shortName)
    param.remove(score.shortName)
    param.put(shortName, fscore)
    param
  }
}
