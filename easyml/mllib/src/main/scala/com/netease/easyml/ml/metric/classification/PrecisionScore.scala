 ...
 */
@Register(alias = Array("precision", "precision_score", "precision-score"))
class PrecisionScore(override val uid: String) extends Metric
  with FScoreBaseParams with DefaultParamsWritable {
  def this() = this(Identifiable.randomUID("precision"))

  override lazy val shortName: String = "precision"
